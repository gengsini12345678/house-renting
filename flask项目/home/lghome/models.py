...
class User(BaseModel, db.Model):
    ...
    # def pwd_hash(self,origin_password):
    #     """
    #     对密码加密
    ...
    def to_dict(self):
        ...
        user_dict = {
            ...
            "name": self.name,
            "mobile": self.mobile,
            "avatar": contants.QINIU_URL_DOMAIN + self.avatar_url if self.avatar_url else "",
...
class House(BaseModel, db.Model):
    ...
    def to_full_dict(self):
        ...
        for image in self.images:
            img_urls.append(contants.QINIU_URL_DOMAIN + image.url)
        ...
        orders = Order.query.filter(Order.house_id == self.id, Order.status == "COMPLETE", Order.comment != None)\
            .order_by(Order.update_time.desc()).limit(contants.HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS)
        for order in orders:
            comment = {
...
class Order(BaseModel, db.Model):
    ...
    __tablename__ = "h_order_info"

    id = db.Column(db.Integer, primary_key=True)  # 订单编号
    user_id = db.Column(db.Integer, db.ForeignKey("h_user_profile.id"), nullable=False)  # 下订单的用户编号
    house_id = db.Column(db.Integer, db.ForeignKey("h_house_info.id"), nullable=False)  # 预订的房间编号
